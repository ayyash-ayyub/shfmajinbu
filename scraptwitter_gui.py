import sys
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QPushButton, QVBoxLayout, QFileDialog, QHBoxLayout, QMessageBox
import pandas as pd
import os
...
class TwitterScraperGUI(QWidget):
    ...
    def run_search(self):
        ...
        # Validate inputs
        if not username or not keyword or not from_date or not to_date or not lang:
            QMessageBox.warning(self, "Input Error", "All fields must be filled out.")
            return

        try:
            # Build the search keyword for tweet-harvest
            search_keyword = f'from:{username} {keyword} since:{from_date} until:{to_date} lang:{lang}'
